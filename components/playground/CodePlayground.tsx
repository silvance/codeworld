...
export default function CodePlayground() {
  ...
  // Keep ref fresh for Monaco keybinding
  ...
  return (
    <div className="flex flex-col h-screen bg-zinc-950 text-zinc-100 overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-zinc-800 bg-zinc-900 flex-shrink-0">
        ...
        <button
          onClick={() => { setOutput(''); setExecMs(null) }}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="h-44 flex flex-col border-t border-zinc-800 flex-shrink-0">
            ...
            <div className="flex items-center px-3 py-1 bg-zinc-900 border-b border-zinc-800 flex-shrink-0">
              ...
              {language === 'python' && (
                <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${
                  ...
                  pyStatus === 'error'   ? 'text-red-400     bg-red-950'    :
                  'text-zinc-600'
                }`}>
                  pyodide {pyStatus}
                </span>
              ...
              {language === 'javascript' && (
                <span className="text-[10px] font-mono text-zinc-700">browser JS</span>
              )}
              {language === 'bash' && (
                <span className="text-[10px] font-mono text-zinc-700">simulated</span>
              )}
            </div>
            ...
            <pre className={`flex-1 overflow-auto px-3 py-2 text-xs font-mono leading-relaxed bg-zinc-950 ${
              ...
              {output
                ? output
                : <span className="text-zinc-700">press ▶ run  or  {navigator.platform.includes('Mac') ? '⌘' : 'Ctrl'}+Enter</span>
              }
            </pre>
          </div>
      ...
      <div className="flex items-center px-3 py-0.5 border-t border-zinc-800 bg-zinc-900 flex-shrink-0">
        ...
        {language === 'bash' && (
          <>
            <span className="mx-2 text-zinc-800">·</span>
            <span className="text-[10px] font-mono text-zinc-700">
              simulated env — no real execution
            </span>
          </>
        )}
      </div>
